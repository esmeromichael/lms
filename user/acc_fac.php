		...
		<?php 
										...
										if($cpassword!=($_POST["password"]))
											...
											$account_query=mysql_query("select * from accounts where id_number='".$_POST['id']."'")or die(mysql_error());
												...
												echo "<script language='javascript' type='text/javascript'> location.href='faculty_login.php' </script>";	
												}
												else {
													mysql_query("INSERT INTO `accounts` (type, id_number, password, date_register) VALUES ('faculty','".$row1['id']."','$password', now())")
												...
												<div class="alert alert-danger">ID Number is not faculty. Please check your ID Number first!</div>
											<?php
											}
										}
										}										
